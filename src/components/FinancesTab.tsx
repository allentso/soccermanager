...
export default function FinancesTab({ gameState }: FinancesTabProps) {
  ...
  const financeItems = [
    { label: t('finances.clubBalance'), value: myTeam.finance, color: myTeam.finance >= 0 ? "text-primary-500" : "text-red-500" },
    { label: t('finances.wageBudget'), value: myTeam.wage_budget, color: "text-gray-800 dark:text-gray-200" },
    { label: t('finances.transferBudget'), value: myTeam.transfer_budget, color: "text-gray-800 dark:text-gray-200" },
    { label: t('finances.seasonIncome'), value: myTeam.season_income, color: "text-primary-500" },
    { label: t('finances.seasonExpenses'), value: myTeam.season_expenses, color: "text-red-500" },
  ];
        ...
        <CardBody>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <div className="bg-gray-50 dark:bg-navy-800 rounded-xl p-4 text-center">
              <p className="text-xs font-heading font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-1">{t('finances.squadValue')}</p>
      ...
      <Card>
        ...
        <CardBody>
          <div className="text-center mb-4">
            <p className="text-xs font-heading font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">{t('finances.weeklyTotal')}</p>
            <p className="font-heading font-bold text-2xl text-gray-800 dark:text-gray-100 mt-1">{formatVal(totalWages)}/wk</p>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
              {t('finances.budget')}: {formatVal(myTeam.wage_budget)}/wk — {totalWages <= myTeam.wage_budget
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 dark:bg-navy-800 border-b border-gray-200 dark:border-navy-600 text-xs">
                  <th className="py-3 px-5 font-heading font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">{t('common.player')}</th>
                  ...
                  <th className="py-3 px-5 font-heading font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">{t('finances.wagePerWeek')}</th>
                  <th className="py-3 px-5 font-heading font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">{t('finances.marketValue')}</th>
                  <th className="py-3 px-5 font-heading font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">{t('common.contract')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 dark:divide-navy-600">
                {[...roster].sort((a, b) => b.wage - a.wage).slice(0, 10).map(p => (
                  <tr key={p.id} className="hover:bg-gray-50 dark:hover:bg-navy-700/50 transition-colors">
                    ...
                    <td className="py-3 px-5 text-sm text-gray-500 dark:text-gray-400">{p.contract_end ? t('finances.until', { year: p.contract_end.substring(0, 4) }) : "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
