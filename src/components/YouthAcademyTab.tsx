...
export default function YouthAcademyTab({ gameState, onSelectPlayer }: YouthAcademyTabProps) {
  ...
  const avgOvr = youthPlayers.length > 0 ? Math.round(youthPlayers.reduce((s, p) => s + p.ovr, 0) / youthPlayers.length) : 0;
  const avgPotential = youthPlayers.length > 0 ? Math.round(youthPlayers.reduce((s, p) => s + p.potential, 0) / youthPlayers.length) : 0;
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-2.5 px-4 text-center">
                        <span className="text-sm font-heading font-bold text-gray-700 dark:text-gray-300 tabular-nums">{player.age}</span>
                      </td>
                      <td className="py-2.5 px-4 text-center">
                        <span className="text-sm font-heading font-bold text-gray-800 dark:text-gray-100 tabular-nums">{player.ovr}</span>
                      </td>
                      <td className="py-2.5 px-4 text-center">
                        <span className={`text-sm font-heading font-bold tabular-nums ${potLabel.color}`}>{player.potential}</span>
                        <p className={`text-[9px] font-heading uppercase tracking-wider ${potLabel.color}`}>{potLabel.label}</p>
                      </td>
                      <td className="py-2.5 px-4">
                        <div className="flex items-center gap-2">
                          <ProgressBar value={Math.min(100, (player.ovr / player.potential) * 100)} variant={growthRoom > 15 ? "accent" : growthRoom > 5 ? "primary" : "auto"} size="sm" />
                          <span className="text-[10px] font-heading font-bold text-gray-500 tabular-nums w-6">+{growthRoom}</span>
                        </div>
                      </td>
                      <td className="py-2.5 px-4">
