      ...
      {events.length === 0 ? (
        ...
        events.map((evt, i) => {
          ...
          return (
            <div
              key={i}
              className={`flex items-start gap-3 px-3 py-2 rounded-lg transition-colors ${display.important ? "bg-navy-800/80" : "opacity-60"}`}
            >
              <span className="text-gray-600 tabular-nums font-heading text-sm w-8 text-right flex-shrink-0 pt-0.5">
                {evt.minute}'
              </span>
        ...
        return (
          <div key={i}>
            <div className="flex justify-between text-xs mb-1">
              ...
              <span className="text-gray-500 font-heading uppercase tracking-wider text-[10px]">
                {stat.label}
              </span>
        ...
        {positions.map((pos) => {
          ...
          return (
            <div key={pos} className="mb-3">
              <p className="text-[10px] font-heading uppercase tracking-widest text-gray-600 mb-1">
                {pos}s
              </p>
              {players.map((p) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`font-medium flex-1 truncate ${isOff ? "line-through text-gray-600" : "text-gray-300"}`}
                    >
                      {p.name}
                    </span>
                    {yc > 0 && (
                    ...
                    <div className="w-14 flex items-center gap-1">
                      ...
                      <span className="text-gray-500 tabular-nums text-[10px] w-6 text-right">
            ...
            {bench.map((p) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-gray-400 font-medium flex-1 truncate">
                    {p.name}
                  </span>
                  <Badge variant="neutral" size="sm">
                    {translatePositionAbbreviation(t, p.position)}
                  </Badge>
                  <span className="text-gray-600 tabular-nums text-[10px] w-6 text-right">
                    {Math.round(p.condition)}
                  </span>
                </div>
              );
