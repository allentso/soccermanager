...
export default function Dashboard() {
  ...
  const hasMatchToday =
    gameState?.league?.fixtures.some((f) => {
      const today = gameState.clock.current_date.split("T")[0];
      return (
        f.date === today &&
        f.status === "Scheduled" &&
        (f.home_team_id === gameState.manager.team_id ||
          f.away_team_id === gameState.manager.team_id)
      );
    }) ?? false;
  ...
  const unreadMessagesCount =
    gameState.messages?.filter((m) => !m.read).length || 0;
  const myTeamName =
    gameState.teams.find((tm) => tm.id === gameState.manager.team_id)?.name ??
    null;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-navy-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-navy-600 w-full max-w-md p-6 mx-4">
            <div className="flex items-center gap-3 mb-4">
              <div
                className={`w-10 h-10 rounded-xl bg-gradient-to-br ${MODE_META[matchMode]?.color} flex items-center justify-center text-white`}
              >
            ...
            {(() => {
              ...
              return (
                <div className="bg-gray-50 dark:bg-navy-700 rounded-xl p-4 mb-4 text-center">
                  <p className="text-xs font-heading uppercase tracking-widest text-gray-400 mb-2">
                    {t("common.matchday", { n: fixture.matchday })}
                  </p>
                  <p className="text-lg font-heading font-bold text-gray-900 dark:text-white">
                    {homeName}{" "}
                    <span className="text-gray-400 mx-2">{t("common.vs")}</span>{" "}
                    {awayName}
                  </p>
                </div>
              );
            ...
            {searchOpen && searchQuery.length >= 2 && (
              <div className="absolute top-full mt-1 left-0 right-0 bg-white dark:bg-navy-700 rounded-lg shadow-xl border border-gray-200 dark:border-navy-600 z-30 max-h-80 overflow-y-auto">
                {(() => {
                  ...
                  const matchedPlayers = gameState.players
                    .filter(
                      (p) =>
                    ...
                    .slice(0, 5);
                  const matchedTeams = gameState.teams
                    .filter(
                      (t) =>
                  ...
                  return (
                    <>
                      {matchedTeams.length > 0 && (
                        <div>
                          <p className="px-3 pt-2 pb-1 text-xs font-heading font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">
                          ...
                          {matchedTeams.map((t) => (
                            <button
                              ...
                              className="w-full text-left px-3 py-2 hover:bg-gray-50 dark:hover:bg-navy-600 flex items-center gap-2 transition-colors"
                            >
                              <div
            ...
            <div className="relative">
              <div className="flex">
                ...
                >
                  ...
                  <ChevronRight
                    className={`w-4 h-4 ${isAdvancing ? "animate-pulse" : ""}`}
                  />
                </button>
                <button
              ...
              {showContinueMenu && (
                <div className="absolute right-0 top-full mt-1 w-64 bg-white dark:bg-navy-700 rounded-lg shadow-xl border border-gray-200 dark:border-navy-600 py-1 z-20">
                  {(["live", "spectator", "delegate"] as const).map((mode) => (
                    ...
                    >
                      <span
                        className={`${matchMode === mode ? "text-primary-500" : "text-gray-400"}`}
                      >
                        {MODE_META[mode]?.icon}
                      </span>
                      <div className="flex-1">
                        <span className="font-heading font-bold text-gray-800 dark:text-gray-100 uppercase tracking-wide text-xs">
        ...
        <div className="flex-1 overflow-auto p-6 bg-gray-100 dark:bg-navy-900">
          ...
          {!selectedPlayerId &&
            ...
            (() => {
              ...
              const urgentUnread = (gameState.messages || []).filter(
                (m) => !m.read && m.priority === "Urgent",
              ).length;
              const startingXi = myTeam?.starting_xi_ids || [];
              const xiPlayersOnRoster = startingXi.filter((id) =>
                roster.some((p) => p.id === id),
              );
              const xiCount = xiPlayersOnRoster.length;
              const injuredInXi = xiPlayersOnRoster.filter(
                (id) => roster.find((p) => p.id === id)?.injury,
              ).length;
              ...
              if (exhausted >= 3)
                alerts.push({
                  id: "exhausted",
                  text: `${exhausted} players in critical condition (<25%)`,
                  tab: "Training",
                  severity: "warn",
                });
              if (injured >= 2)
              ...
              if (startingXi.length > 0) {
                if (injuredInXi > 0)
                  alerts.push({
                    id: "injured_xi",
                    text: `${injuredInXi} injured player${injuredInXi > 1 ? "s" : ""} in Starting XI — replace them`,
                    tab: "Squad",
                    severity: "warn",
                  });
                if (
              ...
              if (urgentUnread > 0)
                alerts.push({
                  id: "urgent",
                  text: `${urgentUnread} urgent message${urgentUnread > 1 ? "s" : ""} unread`,
                  tab: "Inbox",
                  severity: "warn",
                });
