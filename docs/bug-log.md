...
| [REQ-20260611-02](#req-20260611-02--球队降级时强制玩家经理辞职) | 需求 | P2 | 降级强制辞职 | 顶级联赛降级 → 失业 + 履历 relegated + AI 立即接管 |
...
| 编号 | 类型 | 状态 | 严重程度 | 标题 | 要点 |
|------|------|------|----------|------|------|
| [REQ-20260611-01](#req-20260611-01--完善租借系统并新增中超第六联赛) | 需求 | 部分完成 | P2 | 完善租借 + 中超 | 挂牌/召回/续租/UI/AI/窗外下架；**中超未做** |
...
| 状态 | 部分完成 |
| 严重程度 | P2 |
| 模块 | `scripts/systems/transfer_manager.lua`（租借）、`scripts/ui/screens/market/loans_tab.lua`、`scripts/ui/screens/player_detail.lua`、`scripts/ui/screens/squad.lua`、`scripts/ui/screens/training.lua`；**中超**：`scripts/data/`、`scripts/systems/world_generator.lua`（未做） |
| 发现人 | 玩家反馈 |
| 修复人 | |
...
- 可选：二级联赛降级是否同样强制辞职（建议仅顶级联赛降级触发，待设计确认）
...
- 提交/PR：
- 相关记录：`REQ-20260611-08`（次级联赛可玩 — 与本需求「降级后不可继续执教原队」一致）
- 备注：仅**顶级联赛**降级触发；二级储备池降级不在此流程（当前无玩家执教次级路径）
...
- 提交/PR：
- 相关记录：REQ-20260611-02（降级强制辞职 — 若实施需决定降级后是否仍可在次级执教）、REQ-20260611-01（中超为 **顶级** 第六联赛，与本需求层级不同）
