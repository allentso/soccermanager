...
pub fn apply_player_response(
    ...
    // Determine morale delta based on event type and chosen response
    let (delta, description) = if message_id.starts_with("morale_talk_") {
        match option_id {
            "encourage" => {
                let d = rng.gen_range(8..=15);
                (d, format!("Player feels encouraged. Morale +{}", d))
            }
            "promise_time" => {
                let d = rng.gen_range(10..=18);
                (d, format!("Player is reassured about playing time. Morale +{}", d))
            }
            "work_harder" => {
                let d = rng.gen_range(-5..=8);
                (
                    d,
                    if d >= 0 {
                        ...
                        format!(
                            "Player is hurt by the tough love. Morale {}",
                            d
                        )
                    },
                )
        ...
        match option_id {
            "explain" => {
                ...
                (
                    d,
                    format!("Player understands the situation. Morale +{}", d),
                )
            }
            "promise_chance" => {
                ...
                (
                    d,
                    format!("Player is excited about the opportunity. Morale +{}", d),
                )
            }
            "prove_yourself" => {
                let d = rng.gen_range(-3..=10);
                (
                    d,
                    if d >= 0 {
        ...
        match option_id {
            "praise_back" => {
                let d = rng.gen_range(3..=6);
                (d, format!("Player thrives on the praise. Morale +{}", d))
            }
            "stay_professional" => {
                let d = rng.gen_range(0..=3);
                (d, format!("Player appreciates the professionalism. Morale +{}", d))
            }
            "higher_expectations" => {
                ...
                (
                    ...
                    if d >= 0 {
                        ...
                        format!("Player feels the pressure. Morale {}", d)
                    },
                )
            }
            _ => return None,
        ...
        match option_id {
            "reassure" => {
                let d = rng.gen_range(5..=12);
                (d, format!("Player is reassured about their future. Morale +{}", d))
            }
            "noncommittal" => {
                let d = rng.gen_range(-4..=2);
                (
                    d,
                    if d >= 0 {
                        format!("Player accepts the uncertainty. Morale +{}", d)
                    } else {
                        format!("Player is unsettled. Morale {}", d)
                    },
                )
            }
            "no_renewal" => {
                let d = rng.gen_range(-12..=-5);
                (
                    d,
                    format!("Player is devastated. Morale {}", d),
                )
            }
            _ => return None,
        }
    ...
    if let Some(player) = game.players.iter_mut().find(|p| p.id == player_id) {
        ...
        player.morale = (base + delta as i16).clamp(10, 100) as u8;
    }
