...
pub fn compute_season_awards(game: &Game) -> SeasonAwards {
    ...
    let mut entries: Vec<(String, String, String, String, &domain::player::PlayerSeasonStats, i32)> = Vec::new();
    for player in &game.players {
        ...
        // Calculate age
        let age = if let Ok(dob) = chrono::NaiveDate::parse_from_str(&player.date_of_birth, "%Y-%m-%d") {
            let today = current_date.date_naive();
            let mut a = today.year() - dob.year();
            if today.ordinal() < dob.ordinal() { a -= 1; }
            a
        } else {
            30
        };
    ...
    let gk_ids: std::collections::HashSet<String> = game.players.iter()
